...
function processTowerDataFile(td) {
  ...
  let lines = linesWithSpace.map((str) =>
...
function getNumberofFaces(towerData) {
  ...
  // getPlanBracingPostions(towerData)
...
function checkInvertedPanels(towerData) {
  towerData.forEach((line) => {
    if (
      line.join(' ').match(/^(?=.*\bFACE\b)(?=.*\bINVERT|INV(?:ERTED)?\b).*/)
    ) {
...
// let planBracingPositions = []

// function getPlanBracingPostions(towerData) {
//     towerData.forEach((line, lineNumber) => {
//         if (line.join(' ').match(/.*\bFACE\b.*\bLEG\b.*\bBR1?\b/)) {
